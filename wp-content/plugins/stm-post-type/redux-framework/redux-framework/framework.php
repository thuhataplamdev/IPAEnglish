...
if ( ! class_exists( 'ReduxFramework' ) ) {
	...
	class ReduxFramework {
		...
		public static function init() {
			...
			if ( true == self::$_is_plugin || true == self::$_as_plugin ) {
		...
		public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
			...
			if ( 'customize' == $GLOBALS['pagenow'] && isset( $_GET['theme'] ) && ! empty( $_GET['theme'] ) ) {
		...
		public function set_options( $value = '' ) {
			...
			if ( ! empty( $value ) ) {
				...
				} elseif ( 'theme_mods' === $this->args['database'] ) {
					set_theme_mod( $this->args['opt_name'] . '-mods', $value );
				} elseif ( 'theme_mods_expanded' === $this->args['database'] ) {
		...
		public function get_wordpress_data( $type = false, $args = array() ) {
			...
			if ( empty( $data ) && ! empty( $type ) ) {
				...
				if ( ! empty( $type ) && empty( $data ) ) {
					...
					} elseif ( 'tags' == $type || 'tag' == $type ) { // NOT WORKING!
						$tags = get_tags( $args );
						if ( ! empty( $tags ) ) {
							foreach ( $tags as $tag ) {
					...
					} elseif ( 'image_size' == $type || 'image_sizes' == $type ) {
					...
					} elseif ( 'elusive-icons' == $type || 'elusive-icon' == $type || 'elusive' == $type ||
					'font-icon' == $type || 'font-icons' == $type || 'icons' == $type
					) {
		...
		private function _default_cleanup() {
			...
			if ( '' == $this->args['global_variable'] && false !== $this->args['global_variable'] ) {
			...
			if ( isset( $this->args['customizer_only'] ) && true == $this->args['customizer_only'] ) {
			...
			if ( 'submenu' == $this->args['menu_type'] ) {
				...
				if ( true === $this->args['allow_sub_menu'] ) {
					foreach ( $this->sections as $k => $section ) {
						$canBeSubSection = ( $k > 0 && ( ! isset( $this->sections[ ( $k ) ]['type'] ) || 'divide' != $this->sections[ ( $k ) ]['type'] ) ) ? true : false;
						if ( ! isset( $section['title'] ) || ( $canBeSubSection && ( isset( $section['subsection'] ) && true == $section['subsection'] ) ) ) {
							continue;
						}
		...
		public function _admin_bar_menu() {
			...
			if ( ! is_super_admin() || ! is_admin_bar_showing() || ! $this->args['admin_bar'] || 'hidden' == $this->args['menu_type'] ) {
				return;
			}
		...
		public function _output_css() {
			if ( false == $this->args['output'] && false == $this->args['compiler'] ) {
				return;
			}
			...
			if ( ! empty( $this->outputCSS ) && ( true == $this->args['output_tag'] || ( isset( $_POST['customized'] ) ) ) ) {
				//phpcs:ignore
				...
				if ( $this->args['async_typography'] && ! empty( $this->typography ) ) {
					...
					?>
						<script>
							...
							WebFontConfig['google'] = {families: [<?php /*phpcs:ignore */ echo $typography->makeGoogleWebfontString( $this->typography ); ?>]};
		...
		public function get_header_html( $field ) {
			...
			if ( true == $this->args['default_show'] && isset( $field['default'] ) && isset( $this->options[ $field['id'] ] ) && $this->options[ $field['id'] ] != $field['default'] && ! in_array( $field['type'], $filter_arr ) ) {
			...
			foreach ( $this->sections as $k => $section ) {
				...
				if ( isset( $section['fields'] ) ) {
					foreach ( $section['fields'] as $fieldk => $field ) {
						...
						if ( isset( $field['type'] ) && 'info' == $field['type'] && $sectionIndent ) {
						...
						if ( ! isset( $this->options[ $field['id'] ] ) && isset( $field['default'] ) ) {
							$this->options_defaults[ $field['id'] ] = ( isset( $this->options[ $field['id'] ] ) ) ? $this->options[ $field['id'] ] : null;
							...
							if ( 'sorter' == $field['type'] || 'sortable' == $field['type'] ) {
								$this->options_defaults[ $field['id'] ] = $this->options[ $field['id'] ];
								$this->options[ $field['id'] ]          = $field['options'];
								$doUpdate                               = true;
							}
						}
		...
		public function _validate_options( $plugin_options ) {
			...
			if ( ( isset( $plugin_options['import_code'] ) && ! empty( $plugin_options['import_code'] ) ) || ( isset( $plugin_options['import_link'] ) && ! empty( $plugin_options['import_link'] ) ) ) {
				...
				if ( ! empty( $imported_options ) && is_array( $imported_options ) && isset( $imported_options['redux-backup'] ) && '1' == $imported_options['redux-backup'] ) {
					...
					if ( 'transient' == $this->args['database'] || 'theme_mods' == $this->args['database'] || 'theme_mods_expanded' == $this->args['database'] || 'network' == $this->args['database'] ) {
			...
			unset( $plugin_options['defaults'], $plugin_options['defaults_section'], $plugin_options['import'], $plugin_options['import_code'], $plugin_options['import_link'], $plugin_options['compiler'], $plugin_options['redux-section'] );
			if ( 'transient' == $this->args['database'] || 'theme_mods' == $this->args['database'] || 'theme_mods_expanded' == $this->args['database'] ) {
				$this->set_options( $plugin_options );
		...
		public function section_menu( $k, $section, $suffix = '', $sections = array() ) {
			...
			$string = '';
			if ( ( ( isset( $this->args['icon_type'] ) && 'image' == $this->args['icon_type'] ) || ( isset( $section['icon_type'] ) && 'image' == $section['icon_type'] ) ) || ( isset( $section['icon'] ) && strpos( $section['icon'], '/' ) !== false ) ) {
			...
			$canBeSubSection = ( $k > 0 && ( ! isset( $sections[ ( $k ) ]['type'] ) || 'divide' != $sections[ ( $k ) ]['type'] ) ) ? true : false;

			if ( ! $canBeSubSection && isset( $section['subsection'] ) && true == $section['subsection'] ) {
			...
			if ( isset( $section['type'] ) && 'divide' == $section['type'] ) {
				...
				if ( $subsections ) {
					...
					while ( $doLoop ) {
						...
						if ( isset( $_GET['page'] ) && $_GET['page'] == $this->args['page_slug'] ) {
							if ( isset( $sections[ $nextK ]['panel'] ) && false == $sections[ $nextK ]['panel'] ) {
								$display = false;
							}
						}

						if ( count( $sections ) < $nextK || ! isset( $sections[ $nextK ] ) || ! isset( $sections[ $nextK ]['subsection'] ) || true != $sections[ $nextK ]['subsection'] ) {
							$doLoop = false;
						} else {
							if ( ! $display ) {
								continue;
							...
							if ( isset( $sections[ $nextK ]['hidden'] ) ) {
								$hide_sub = ( true == $sections[ $nextK ]['hidden'] ) ? ' hidden ' : '';
							}

							if ( ( isset( $this->args['icon_type'] ) && 'image' == $this->args['icon_type'] ) || ( isset( $sections[ $nextK ]['icon_type'] ) && 'image' == $sections[ $nextK ]['icon_type'] ) ) {
								// if( !empty( $this->args['icon_type'] ) && $this->args['icon_type'] == 'image' ) {
		...
		public function _section_desc( $section ) {
			...
			if ( isset( $this->sections[ $id ]['desc'] ) && ! empty( $this->sections[ $id ]['desc'] ) ) {
				echo '<div class="redux-section-desc">' . wp_kses_post( $this->sections[ $id ]['desc'] ) . '</div>';
			}
		}
			...
			if ( isset( $field['type'] ) ) {
				...
				if ( class_exists( $field_class ) ) {
					...
					*/
					...
					echo '<fieldset id="' . esc_attr( $this->args['opt_name'] ) . '-' . esc_attr( $field['id'] ) . '" class="' . esc_attr( $hidden ) . 'redux-field-container redux-field redux-field-init redux-container-' . esc_attr( $field['type'] ) . ' ' . esc_attr( $class_string ) . '" data-id="' . esc_attr( $field['id'] ) . '" ' . esc_attr( $data_string ) . ' data-type="' . esc_attr( $field['type'] ) . '">';
					// }
					//phpcs:ignore
					echo $_render;
					...
					echo ( isset( $field['description'] ) && esc_html( $field['type'] ) != 'info' && esc_html( $field['type'] ) !== 'section' && ! empty( $field['description'] ) ) ? '<div class="description field-desc">' . wp_kses_post( $field['description'] ) . '</div>' : '';
		...
		private function compareValueDependencies( $parentValue, $checkValue, $operation ) {
			...
			switch ( $operation ) {
				...
				case 'empty':
				case '!isset':
					if ( empty( $parentValue ) || '' == $parentValue || null == $parentValue ) {
						$return = true;
					}
					break;
				case 'not_empty':
		...
		private function checkRequiredDependencies( $field, $data ) {
			...
			if ( ! in_array( $data['parent'], $this->fieldsHidden ) && ( ! isset( $this->folds[ $field['id'] ] ) || 'hide' != $this->folds[ $field['id'] ] ) ) {
		 ...
		 * @return  array $result
		 */
		public function redux_parse_str( $string ) {
			if ( '' == $string ) {
				return false;
			}
		 ...
		 * @return  array $merged
		 */
		public function redux_array_merge_recursive_distinct( array $array1, array $array2 ) {
			$merged = $array1;
		...
		private function change_demo_defaults() {
			if ( true == $this->args['dev_mode'] || true == Redux_Helpers::isLocalHost() ) {
