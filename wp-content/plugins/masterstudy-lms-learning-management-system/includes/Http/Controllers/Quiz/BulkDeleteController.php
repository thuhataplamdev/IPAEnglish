...
final class BulkDeleteController {
	...
	public function __invoke( WP_REST_Request $request ): WP_REST_Response {
		...
		$quizzes = (array) $validator->get_validated()['quizzes'];

		if ( empty( $quizzes ) ) {
			return WpResponseFactory::bad_request(
				esc_html__( 'No quizzes were provided for deletion.', 'masterstudy-lms-learning-management-system' )
			);
		}
		...
		try {
			( new QuizAdminRepository() )->bulk_delete( $quizzes );
