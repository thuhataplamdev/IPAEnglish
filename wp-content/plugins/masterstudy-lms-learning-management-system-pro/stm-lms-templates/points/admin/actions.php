			...
			<td class="point-score">
				...
				<div class="edit_points" v-else>
					...
					<div class="actions-edit">
						<i class="stmlms-check1" @click="changePoints(point, user)"></i>
						<i class="stmlms-cross" @click="$set(point, 'editing', false);"></i>
					</div>
				</div>
			</td>
			<td class="point-actions">
				<i class="stmlms-pencil3" @click="$set(point, 'editing', true); $set(point, 'edit', point.score.match(/\d+/g).map(Number) );"></i>
				<i class="stmlms-cross" @click="deletePoint(point, point_index, user);"></i>
			</td>
		</tr>
		</tbody>
		<tfoot>
