...
class STM_LMS_Certificate_Builder {
	...
	public function enqueue() {
		wp_enqueue_script( 'jspdf', STM_LMS_PRO_URL . '/assets/js/certificate-builder/jspdf.umd.js', array(), stm_lms_custom_styles_v(), true );
		wp_enqueue_script( 'pdfjs', STM_LMS_PRO_URL . '/assets/js/certificate-builder/pdf.min.js', array(), stm_lms_custom_styles_v(), true );
		wp_enqueue_script( 'pdfjs_worker', STM_LMS_PRO_URL . '/assets/js/certificate-builder/pdf.worker.min.js', array(), stm_lms_custom_styles_v(), true );
		wp_enqueue_script( 'masterstudy_certificate_fonts', STM_LMS_PRO_URL . '/assets/js/certificate-builder/certificates-fonts.js', array(), stm_lms_custom_styles_v(), true );
	}
	...
	public function admin_page() {
		...
		wp_enqueue_style( 'masterstudy_certificate', STM_LMS_PRO_URL . '/assets/css/certificate-builder/main.css', null, stm_lms_custom_styles_v() );
		wp_enqueue_style( 'masterstudy_certificate_fonts', 'https://fonts.googleapis.com/css?family=Katibeh|Amiri|Merriweather:400,700|Montserrat:400,700|Open+Sans:400,700|Oswald:400,700|M+PLUS+2:400,800', null, stm_lms_custom_styles_v() );
	...
	public function get_certificate() {
		...
		if ( ! empty( $id ) ) {
			...
			foreach ( $fields as $field ) {
				...
				$field              = apply_filters( 'stm_certificates_field', $field );
