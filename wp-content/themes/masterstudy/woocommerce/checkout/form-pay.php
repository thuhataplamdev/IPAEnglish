...
<form id="order_review" method="post">
	...
	<div id="payment">
					...
					if ( ! empty( $available_gateways ) ) {
						foreach ( $available_gateways as $gateway ) {
							...
							if ( $gateway->has_fields() || $gateway->get_description() ) {
										echo '<div class="payment_box payment_method_' . $gateway->id . '" style="display:none;">';  // phpcs:ignore
						...
						echo '<p>' . esc_html__( 'Sorry, it seems that there are no available payment methods for your location. Please contact us if you require assistance or wish to make alternate arrangements.', 'masterstudy' ) . '</p>';
				}
				?>
			</ul>
		<?php endif; ?>
		<div class="form-row">
			...
			echo apply_filters( 'woocommerce_pay_order_button_html', '<input type="submit" class="button alt" id="place_order" value="' . esc_attr( $pay_order_button_text ) . '" data-value="' . esc_attr( $pay_order_button_text ) . '" />' );// phpcs:ignore
			?>
